---
title: "Classiphy User Guide"
author: "Huateng Huang"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Classiphy User Guide}
...
In principle, user can use any program to do the simulation as long as the simulated trees are organized into three files like this. We only considering the situation where one individual is sequenced for each species (like most of phylogenomic studies), so the tip labels in locus and gene trees need to be consistent with those of species tree, and currently, branch length is not used in calculating summary statistics, so the trees can be topology-only.
...
* `repfolder`: the path to one repfolder
* `phylonet.path`: the path to the executable file of PhyloNet (which was used to calculate MDC between gene tree and species trees)
...
Classiphy uses the `dapc` function in R package `adegenet` for classification. This package includes a wrapper function `classifyData` originally written by Jeet Sukumaran in program [archipelago](https://github.com/jeetsukumaran/archipelago)  
...
* `trained.model` The trained model, which includes the information that can be use to assess how the model performed based on the posterior prediction of the training data set. [need more details here]
